...
df = pd.read_csv(
    r'C:\Users\Samuel\OneDrive - Drexel University\Dr. Xiao Scripts Coop 2025\kmer\filterd_kmersRaw_ReadsOver_5_Coverage50%_V2.csv',
    header=0,
    dtype={'Unnamed: 5': str},
    low_memory=False
)
...
df.columns = [
    'Kmer_seq', 
    'Logical_Index',   # was 'Unnamed: 1'
    'hits', 
    'Coverage', 
    'Node', 
    'Nodes:Probes',    # was 'Unnamed: 5'
    'End', 
    'Number of Ends',  # was 'Unnamed: 7'
    'ID'
]
...
def nodeProbecalculator(flst):
    ...
    for group in flst:
        ...
        for comparison in group:
            #matches = re.findall(r'[A-Z]_(\d+)_', comparison)
            matches = re.findall(r'([A-Z]+\d+)_', comparison)
            if len(matches) == 2:
...
if __name__ == '__main__':
    ...
    df_out.to_excel(r"C:\Users\Samuel\OneDrive - Drexel University\Dr. Xiao Scripts Coop 2025\kmer\pacbio_sequences.xlsx", index=False, header=True)
